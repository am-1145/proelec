...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-bg-dark relative overflow-x-hidden">
      ...
      {/* ====== NAVBAR ====== */}
      <header role="banner" className="sticky top-0 z-40 bg-bg-card/80 backdrop-blur-xl border-b border-border/50 shadow-sm">
      ...
      <main id="main-content" tabIndex="-1" className="outline-none">
        ...
        <section className="relative z-10 px-5 py-20" aria-labelledby="features-title">
          <div className="max-w-6xl mx-auto">
            ...
            <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }}
              className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" role="list">
              {FEATURES.map((f, i) => (
        ...
        {/* ====== HOW IT WORKS ====== */}
        <section className="relative z-10 px-5 py-20 bg-bg-card/30 border-y border-border/30" aria-labelledby="process-title">
          <div className="max-w-5xl mx-auto">
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center mb-12">
              <p className="text-xs text-secondary font-semibold uppercase tracking-widest mb-2">Simple Process</p>
              <h2 id="process-title" className="text-2xl sm:text-3xl font-bold text-text-primary">How It Works</h2>
              <p className="text-text-muted text-sm mt-2">Four simple steps to become a confident voter</p>
            </motion.div>

            <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }}
              className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5" role="list">
        ...
        {/* ====== CTA ====== */}
        <section className="relative z-10 px-5 py-20" aria-label="Get Started">
          <div className="max-w-3xl mx-auto text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              className="glass-card p-8 sm:p-12 border-primary/20">
              <div className="w-16 h-16 rounded-2xl bg-bg-elevated flex items-center justify-center mx-auto mb-5 text-2xl shadow-xl" aria-hidden="true">
                🗳️
              </div>
      ...
      {/* ====== FOOTER ====== */}
      <footer role="contentinfo" className="relative z-10 border-t border-border/50 bg-bg-card/50">
        <div className="max-w-6xl mx-auto px-5 py-10">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-8 h-8 rounded-lg bg-bg-elevated flex items-center justify-center text-xs shadow-md" aria-hidden="true">🗳️</div>
