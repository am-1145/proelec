...
/**
 * AuthPage Component - Handles User Authentication (Login, Register, Google Sign-In).
 * Implements strict accessibility standards and comprehensive code documentation.
 * @returns {JSX.Element}
 */
export default function AuthPage() {
  ...
  /** @type {['login'|'register', Function]} mode - Current authentication mode. */
  const [mode, setMode] = useState('login');

  /** @type {[boolean, Function]} showPw - Toggle for password visibility. */
  const [showPw, setShowPw] = useState(false);

  /** @type {[boolean, Function]} loading - Loading state for email/password auth. */
  const [loading, setLoading] = useState(false);

  /** @type {[boolean, Function]} googleLoading - Loading state for Google Sign-In. */
  const [googleLoading, setGoogleLoading] = useState(false);

  /** @type {[Object, Function]} form - Form data state. */
  const [form, setForm] = useState({ name: '', email: '', password: '' });

  /**
   * Handles email/password form submission.
   * Validates input, calls the auth API, and manages navigation on success.
  ...
  /**
   * Handles Firebase Google Sign-In.
    ...
    <div className="min-h-screen bg-bg-dark relative overflow-hidden flex items-center justify-center px-4 py-8" role="main" id="main-content">
      ...
      <motion.div
        ...
        <header className="text-center mb-8">
          <div className="w-14 h-14 rounded-2xl bg-bg-elevated flex items-center justify-center mx-auto mb-4 text-2xl shadow-xl shadow-primary/20" aria-hidden="true">
            🗳️
          </div>
        ...
        {/* Auth Card */}
        <section className="glass-card overflow-hidden" aria-labelledby="auth-title">
          <h2 id="auth-title" className="sr-only">{mode === 'login' ? 'Sign In' : 'Create Account'}</h2>
          <div className="h-1 bg-gradient-to-r from-[#FF9933] via-white to-[#138808]" aria-hidden="true" />

          <div className="p-7">
            {/* Tab Switcher */}
            <nav className="flex rounded-xl bg-bg-elevated p-1 mb-6" aria-label="Auth selection">
            ...
            <motion.button
              ...
              aria-label={googleLoading ? 'Signing in with Google' : mode === 'login' ? 'Sign in with Google' : 'Sign up with Google'}
              ...
              title={!isFirebaseConfigured ? 'Google Sign-In requires Firebase configuration' : ''}
            >
              {googleLoading ? (
            ...
            <div className="flex items-center gap-3 mb-5" aria-hidden="true">
              ...
              <span className="text-text-muted text-xs font-inter uppercase tracking-widest">or</span>
              <div className="flex-1 h-px bg-border" />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4" aria-label={mode === 'login' ? 'Login form' : 'Registration form'}>
                ...
                {mode === 'register' && (
                  <motion.div key="name-field"
                    ...
                    <div className="relative">
                      <FiUser className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" size={14} aria-hidden="true" />
                      <input id="auth-name" type="text" className="input-field pl-10" placeholder="Ankit Kumar"
                        value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                        required={mode === 'register'} autoComplete="name" />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div>
                ...
                <div className="relative">
                  <FiMail className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" size={14} aria-hidden="true" />
                  <input id="auth-email" type="email" className="input-field pl-10" placeholder="you@example.com"
              ...
              <div>
                ...
                <div className="relative">
                  <FiLock className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" size={14} aria-hidden="true" />
                  <input id="auth-password" type={showPw ? 'text' : 'password'} className="input-field px-10"
                ...
                <p id="password-hint" className="text-[10px] text-text-muted mt-1 ml-1">
                  {mode === 'register' ? 'Choose a strong password with at least 6 characters.' : 'Enter your registered password.'}
                </p>
              </div>
            ...
            <p className="text-center text-xs text-text-muted mt-5">
              ...
              <button onClick={() => setMode(mode === 'login' ? 'register' : 'login')}
                className="text-primary hover:underline font-medium focus:outline-none focus:ring-1 focus:ring-primary rounded px-1">
        ...
        <footer className="mt-8">
          ...
          <div className="flex items-center justify-center gap-4 mt-3 text-text-muted text-[10px] uppercase tracking-tighter opacity-60">
