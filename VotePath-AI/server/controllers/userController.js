...
const initUser = asyncHandler(async (req, res) => {
  console.log('userController.js: initUser started');
  try {
    ...
    console.log('userController.js: initUser succeeded');
    res.status(201).json({
      success: true,
      data: {
        user,
    ...
    console.error('userController.js: initUser then', e);
    throw e;
  }
});
...
const getUser = asyncHandler(async (req, res) => {
  console.log('userController.js: getUser started');
  try {
    const user = await User.findById(req.params.userId);
    if (!user) {
      return res.status(404).json({ success: false, error: 'User not found' });
    }
    console.log('userController.js: getUser succeeded');
    res.json({ success: true, data: user });
  } catch (e) {
    console.error('userController.js: getUser then', e);
    throw e;
  }
});
